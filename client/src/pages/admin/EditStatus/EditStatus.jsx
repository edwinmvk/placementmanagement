...
const EditStatus = () => {
          ...
          <div className="flex flex-wrap justify-center">
            <Dropdown menu={menuProps}>
              <Button
                ...
                style={{ color: "white" }}
                onClick={() => setStatusDetails(record)}
              >
                <div className="flex items-center gap-x-2">
                  Choose Actions
                  <DownOutlined />
  ...
  const handleMenuClick = (event) => {
    const selectedItem = items.find((item) => item.key === event.key);
    message.info(selectedItem.label);
    // // this also updated the state
    // setstatedata((prev) => {
    //   const stateToChangeIndex = prev.findIndex(
    //     (item) => item.creator.userid === statusDetails.creator.userid
    //   );
    //   const tochangevalues = { ...prev[stateToChangeIndex] };
    //   return prev.splice(stateToChangeIndex, 1, {
    //     ...tochangevalues,
    //     status: selectedItem.label,
    //   });
    // });

    const copystatedata = [...statedata]; // we first make a copy of the statedatarray
    // we then find the index of the data to be updated
    const stateToChangeIndex = copystatedata.findIndex(
      (item) => item.creator.userid === statusDetails.creator.userid
    );
    const tochangevalues = { ...copystatedata[stateToChangeIndex] }; // make a copy of that particular object whose status is to be updated
  ...
  const items = [
    {
      label: "Eligible for Round 1",
      key: "1",
      icon: <HourglassOutlined />,
      style: { color: "purple" },
    },
    {
      label: "Eligible for Round 2",
      key: "2",
      icon: <HourglassOutlined />,
      style: { color: "purple" },
    },
